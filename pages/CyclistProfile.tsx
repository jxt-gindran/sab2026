    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-pale">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="font-black text-brand-navy uppercase tracking-widest text-xs">Loading Profile…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-pale px-4">
        <div className="text-center">
          ...
          <p className="text-brand-slate font-medium mb-8">This cyclist profile doesn't exist or has been removed.</p>
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="relative bg-brand-navy overflow-hidden pt-24 pb-0">
        ...
        {cyclist.profileUrl && (
          <div className="absolute inset-0 z-0">
            <img src={cyclist.profileUrl} alt={cyclist.name} className="w-full h-full object-cover opacity-20 blur-sm scale-105" />
          ...
          <div className="flex flex-col md:flex-row items-end gap-8 pb-0">
            ...
            <div className="relative shrink-0 self-center md:self-end">
              <div className="w-40 h-40 md:w-48 md:h-48 rounded-3xl overflow-hidden border-4 border-brand-cyan shadow-2xl shadow-brand-cyan/20 bg-slate-800">
                {cyclist.profileUrl ? (
                  <img src={cyclist.profileUrl} alt={cyclist.name} className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="flex-grow text-center md:text-left pb-8">
              <div className="text-brand-cyan text-[10px] font-black uppercase tracking-[0.3em] mb-2">SAB 2026 Rider</div>
              ...
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-6 text-white/60 text-xs font-bold uppercase tracking-widest">
                <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-brand-cyan" /> Borneo 2026</span>
                <span className="flex items-center gap-1.5"><Target className="h-3.5 w-3.5 text-brand-orange" /> RM {cyclist.goal.toLocaleString()} Goal</span>
                {cyclist.isFeatured && (
                  <span className="flex items-center gap-1.5 text-brand-orange"><CheckCircle2 className="h-3.5 w-3.5" /> Featured Rider</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex justify-between items-end mb-3">
              <div>
                ...
                <div className="text-white/40 text-[10px] font-bold uppercase tracking-widest">Raised</div>
              </div>
              <div className="text-right">
                <div className="text-white font-black text-lg font-heading">RM {cyclist.goal.toLocaleString()}</div>
                <div className="text-white/40 text-[10px] font-bold uppercase tracking-widest">Goal</div>
              </div>
            </div>
            ...
            <div className="text-right text-white/40 text-[10px] font-black uppercase tracking-widest">{pct.toFixed(0)}% Reached</div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-14 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {cyclist.story && (
            <div>
              <div className="text-brand-orange font-black text-xs uppercase tracking-[0.2em] mb-4">Their Story</div>
              <div
                className="prose prose-base max-w-none text-brand-slate leading-relaxed"
                style={{ fontFamily: 'inherit' }}
                dangerouslySetInnerHTML={{ __html: cyclist.story }}
              />
            </div>
          )}
          ...
          {galleryImages.length > 0 && (
            <div>
              <div className="text-brand-orange font-black text-xs uppercase tracking-[0.2em] mb-4">Gallery</div>
          ...
          <div className="bg-brand-pale/50 rounded-3xl p-8 border border-brand-grey/20">
            <div className="text-brand-orange font-black text-xs uppercase tracking-[0.2em] mb-5">About The Ride</div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              {[
                { label: 'Distance', value: '680 KM' },
                { label: 'Days', value: '6' },
                { label: 'Route', value: 'KK → Miri' },
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 space-y-4">
            ...
            <div className="bg-brand-navy rounded-3xl p-8 text-white">
              <div className="text-brand-cyan text-[10px] font-black uppercase tracking-[0.25em] mb-2">Support {cyclist.name.split(' ')[0]}</div>
              <p className="text-white/70 text-sm font-medium mb-6 leading-relaxed">
                Your donation directly supports this rider's fundraising goal and funds life-saving paediatric care.
              </p>
              <Link
            ...
            >
              See All Riders <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </div>
        </div>
      </div>
